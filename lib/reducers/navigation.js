 ...
 * @param {Array<string>} products.categorized
 * @param {Array<string>} products.default
 * @param {Array<string>} products.dependentProducts
...
export default async (
    ...
    const shouldProductBeSaved = (ID) => {
        return products.categorized.includes(ID) || products.default.includes(ID) || keepAsItIsProducts.has(ID);
    };
            ...
            navigationFilterWriter.on('afterLastMatch', (/** @type {import('fs').WriteStream} */ writer) => {
                for (const category of defaultCategories) {
                    for (const productID of products.default) {
                         ...
                         */
                        if (products.dependentProducts.includes(productID)) {
                            continue;
                        }
