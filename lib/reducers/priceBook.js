...
export default async (files, allReducedProducts, nonMasters, defaultAmount) => await Promise.all(
    files.map(async (priceBookFile) => {
        ...
        const priceBookFilterWriter = new XMLFilterWriter(priceBookFile, getPostfixFile(priceBookFile))
            .setName('Pricebook reducer-writer');

        priceBookFilterWriter.on('afterLastMatch', (/** @type {import('fs').WriteStream} */ writer) => {
        ...
        await priceBookFilterWriter.startAsync('price-table', 'header', /** @type {import('#types').XMLTagFilter} */(tag, raw) => {
            if (tag.name === 'header') {
