...
export default async (files, allReducedProducts, nonMasters, defaultAmount) => {
    ...
    for (let index = 0; index < files.length; index++) {
        ...
        const inventoryFilterWriter = new XMLFilterWriter(inventoryFile, getPostfixFile(inventoryFile))
        ...
        if (index === files.length - 1) {
            inventoryFilterWriter.on('afterLastMatch', (/** @type {import('fs').WriteStream} */ writer) => {
        ...
        await inventoryFilterWriter.startAsync('record', (/** @type {import('#types').XMLTag} */ tag) => {
