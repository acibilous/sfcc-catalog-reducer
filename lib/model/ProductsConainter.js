...
export default class ProductsContainer {
    ...
    getNonMasterProducts() {
        return /** @type {Array<string>} */ ([
            ...this.standard,
            ...Array.from(this.dependencies).filter(dependancy => !this.dependentVariationGroups.has(dependancy))
        ]);
    }
    ...
    addDependencies(dependencyList, variationGroups) {
        ...
        if (variationGroups) {
            variationGroups.forEach(depenedecy => this.dependentVariationGroups.add(depenedecy));
        }
