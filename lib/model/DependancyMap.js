/**
 * Dependant Parent class created for storing in memory only necessary date that will be used later.
 * 
 * If we would use XMLProductDeclaration class insted, we would store large pice of data in 'raw' field,
 * that contains full XML declaration string. 
 */
export class DependancyParent {
    /**
...
export default class DependancyMap {
    ...
    /**
     * @param {import('#types').XMLProductDefinition | DependancyParent} parrentWithDependencies
     */
    addByParent(parrentWithDependencies) {
        const parent = parrentWithDependencies instanceof DependancyParent
            ? parrentWithDependencies
            : new DependancyParent(parrentWithDependencies);
