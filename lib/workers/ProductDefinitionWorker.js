...
import CategoriesProductsContainer from '#model/CategoriesProductsContainer.js';
...
export default class ProductDefinitionWorker {
     ...
     */
    async filterProductsByCategories(productIDsList, categoryConfigs, { isStandardProductsShouldBeProcessed, onlineFlagCheck }) {
        /**
         * @type {Set<string>}
         */
        ...
        const savedProducts = new CategoriesProductsContainer(categoryConfigs);
        ...
        for (const file of this.productCatalogs) {
            ...
            await matcher.startAsync('product', ({ attributes }, raw) => {
                 ...
                 */
                if (savedProducts.isEveryCategoryFull() && !isStandardProductsShouldBeProcessed) {
                    matcher.terminate();
                }
                ...
                const isWhitelistProduct = productCategory !== undefined;
                ...
                const isCurrentContainerFullForCurrentType = savedProducts.isFullFor(productCategory, product.type);

                const isProductCouldBeSaved = !isCurrentContainerFullForCurrentType && isWhitelistProduct && onlineCheckResult;
                ...
                if (isStandardProductsShouldBeProcessed) {
                     ...
                     */
                    product.dependencies.forEach(dependency => delete possiblyStandardProducts[dependency]);
                }

                if (isProductCouldBeSaved) {
                    product.dependencies.forEach(dependency => savedProductsDependencies.add(dependency));

                    savedProducts.add(productCategory, product.type, product.ID);
                }
            });

            if (savedProducts.isEveryCategoryFull()) {
                break;
            }
        }
         ...
         * @type {Array<string>}
         */
        const unusedStandardProducts = [];

        Object.keys(possiblyStandardProducts).forEach((standardProductID) => {
            const categoryID = possiblyStandardProducts[standardProductID];

            if (savedProducts.isFullFor(categoryID, 'standard')) {
                unusedStandardProducts.push(standardProductID);
            } else {
                savedProducts.add(categoryID, 'standard', standardProductID);
            }

            delete possiblyStandardProducts[standardProductID];
        });

        const savedProductIDs = savedProducts.getAllProductIDsFromEveryCategory();
