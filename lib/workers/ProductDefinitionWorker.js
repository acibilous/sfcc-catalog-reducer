...
export default class ProductDefinitionWorker {
    ...
    async filterProducts(productIDsList, containterConfig, { isStandardProductsShouldBeProcessed, onlineFlagCheck }) {
        ...
        for (const file of this.productCatalogs) {
            ...
            await matcher.startAsync('product', ({ attributes }, raw) => {
                 ...
                 */
                if (this.isContainerFull(savedProducts, containterConfig) && !isStandardProductsShouldBeProcessed) {
                    matcher.terminate();
                }

                const onlineCheckResult = onlineFlagCheck
                    ? this.getOnlineFlagFromRaw(raw) === 'true'
                    : true;

                const isWhitelistProduct = productIDsList.includes(attributes['product-id']);

                const type = this.getProductTypeByRaw(raw);
                
                if (type === 'standard' && isWhitelistProduct) {
                    if (
                        isStandardProductsShouldBeProcessed
                        && onlineCheckResult
                        && !allProductsDependencies.has(attributes['product-id'])
                    ) {
                ...
                const isContainerFullForCurrentType = savedProducts[type].length >= containterConfig[type];
            ...
            if (this.isContainerFull(savedProducts, containterConfig)) {
                break;
            }
        }
     ...
     * @returns {'true' | 'false'}
     */
    getOnlineFlagFromRaw(raw) {
        const match = ONLINE_FLAG_REGEX.exec(raw);
