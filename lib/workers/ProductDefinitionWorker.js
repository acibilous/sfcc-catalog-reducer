...
export default class ProductDefinitionWorker {
     ...
     * @param {boolean} isStandardProductsShouldBeProcessed
     * @returns {Promise<[Array<string>, Array<string>]>}
     */
    async filterProducts(productIDsList, config, isStandardProductsShouldBeProcessed) {
        /**
         * @type {Set<string>}
         */
        ...
        for (const file of this.productCatalogs) {
            ...
            await matcher.startAsync('product', ({ attributes }, raw) => {
                if (this.isContainerFull(savedProducts, config) && !isStandardProductsShouldBeProcessed) {
                    matcher.terminate();
                }
                ...
                if (type === 'standard') {
                    if (isStandardProductsShouldBeProcessed && !allProductsDependencies.has(attributes['product-id'])) {
                ...
                // ? we don't know that 'G05616.15RIC.NAPNERO' is dependancy for some master, becase master is not in the list of products for passed category
                // if (productDependencies.includes('G05616.15RIC.NAPNERO')) {
                //     console.log(attributes['product-id']);
                // }
         ...
         * @type {Array<string>}
         */
        const availableStandardProducts = [...possiblyStandardProducts]; // ! False list

        savedProducts.standard = availableStandardProducts.slice(0, config.standard);

        const unusedStandardProducts = availableStandardProducts.slice(config.standard);
