...
import DependencyMap from '#model/DependancyMap.js';
...
export default class ProductDefinitionWorker {
     ...
     * @param {XMLProductDefinition | import('#model/DependancyMap').DependencyParent} productDetails.product
    ...
    proccessNonStandardProduct({
        ...
        const availiableCategoryForSavingAsCategorized = savedCategorizedProducts.getAvailableUnfilledCategoryForType(
            categories,
            product.type
        );
        ...
        savedCategorizedProducts.add(availiableCategoryForSavingAsCategorized, product);
    }
    ...
    async filterProductsByCategories(
        ...
        const savedDefaultProducts = new ProductsConainter(defaultConfig);
                    ...
                    if (dependencyMap.isDependency(product.ID)) {
                        ...
                        if (!savedCategorizedProducts.isEveryCategoryFull()) {
                            parents.forEach(parent => {
                                ...
                                const patentCategories = whitelistedProductCategories;
    ...
    /**
     * @param {ProductsConainter} container 
