    ...
    async filterProducts(productIDsList, config) {
        ...
        for (const file of this.productCatalogs) {
            ...
            await matcher.startAsync('product', ({ attributes }, raw) => {
                ...
                const isStandardProductShouldBeProcessed = config.standard > 0 && savedProducts.standard.length < config.standard;

                if (type === 'standard' && isStandardProductShouldBeProcessed) {
                    if (!savedProductsDependencies.has(attributes['product-id'])) {
            ...
            savedProducts.standard = possiblyStandardProducts.slice(0, config.standard);
