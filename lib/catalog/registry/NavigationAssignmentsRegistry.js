...
export default class NavigationAssignmentsRegistry extends ProductsRegistry {
    ...
    /**
     * @param {Array<Cache['categories']>} categoryRegistryArray
     */
    appendCategories(categoryRegistryArray) {
        categoryRegistryArray.forEach(categoryRegistry => {
            Object.keys(categoryRegistry).forEach(category => {
                this.cache.categories[category] = this.cache.categories[category] || {
                    products: {}
                };

                Object.assign(this.cache.categories[category].products, categoryRegistry[category].products);
            })
        });
    }
};
