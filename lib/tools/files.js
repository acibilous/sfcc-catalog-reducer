 ...
 */
export const getFilesByPatterns = (pathPatternsArray) => {
    return Promise.all(pathPatternsArray.map(async (pattern) => {
        return globby(pattern).then(files => files.filter(f => !isOutPostfixFile(f)));
