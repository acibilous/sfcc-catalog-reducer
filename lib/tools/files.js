 ...
 * @param {string} string
 */
export const withoutXMLExtension = (string) => {
    return string.slice(0, string.length - XMLExtensionLength);
};
...
export const removePostfix = (file) => {
    return withoutXMLExtension(file).slice(0, file.length - outPostfix) + '.xml';
};
...
export const getFilesByPatterns = async (pathPatternsArray) => {
    ...
    const allFilesInOneArray = arrayOfFilesArray.reduce((accumulator, current) => [...accumulator, ...current], []);
