...
/**
 * @param {import('#types').SrcConfig} originalSrc
 */
export const renameReducedToOriginal = async (originalSrc) => {
    const [navigations, priceBooks, inventories] = await Promise.all([
        getFilesByPatterns(originalSrc.navigations),
...
/**
 * @param {import('#types').SrcConfig} paths
 * @returns {Promise<() => void>}
 */
export const getCleaner = async (paths) => {
    const [navigations, priceBooks, inventories] = await Promise.all([
        getFilesByPatterns(paths.navigations),
    ...
    const navigatoinFolder = path.dirname(navigations[0]); // TODO it could be mutile dirs, not just one
